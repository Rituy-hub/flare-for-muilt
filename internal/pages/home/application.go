...
func GenerateApplicationsTemplate(filter string, options *model.Application) template.HTML {
	...
	for _, app := range appsData.Items {
		...
		app.LanURL = fn.ParseDynamicUrl(app.LanURL)
	...
	if filter != "" {
		...
		for _, bookmark := range parseApps {
			if strings.Contains(strings.ToLower(bookmark.Name), filterLower) || strings.Contains(strings.ToLower(bookmark.URL), filterLower) || strings.Contains(strings.ToLower(bookmark.LanURL), filterLower) || strings.Contains(strings.ToLower(bookmark.Desc), filterLower) {
	...
	for _, app := range apps {
		desc := app.Desc
		effectiveURL := getEffectiveURL(app)
		if desc == "" {
			desc = effectiveURL
		}
		templateURL := effectiveURL
		if strings.HasPrefix(effectiveURL, "chrome-extension://") || options.EnableEncryptedLink {
			templateURL = "/redir/url?go=" + data.Base64EncodeUrl(effectiveURL)
		}
		...
		if strings.HasPrefix(app.Icon, "http://") || strings.HasPrefix(app.Icon, "https://") {
			...
			templateIcon = fn.GetYandexFavicon(effectiveURL, mdi.GetIconByName(app.Icon))
		}
